...
static int test_search(RadixMap *map)
{
  ...
  for(i = map->end / 2; i < map->end; i++) {
    ...
    check(found->data.key == d->data.key, "Got the wrong result: %p:%u looking for %u at %u", found,
...
static char *test_operations()
{
  ...
  mu_assert(make_random(map), "Diddn't make a random fake radix map.");

  RadixMap_sort(map);
  mu_assert(check_order(map), "Failed to properly sort RadixMap.");

  mu_assert(test_search(map), "Failed to serach test.");
  ...
  while(map->end > 0) {
    ...
    // test that the end is now the old value, but uint32 max so it trails of
